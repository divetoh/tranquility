...
async def test_user_get_demo(db: AsyncSession, cleandb) -> None:
    ...
    await cr_rand_user(db, created_dt=created_dt)
    users = await crud.user.get_demo_users(db)
    assert len(users) == 0
    ...
    await cr_rand_user(db, email="a@nottest.test", created_dt=created_dt)
    users = await crud.user.get_demo_users(db)
    assert len(users) == 2

    await cr_rand_user(db, is_superuser=True, created_dt=created_dt)
    users = await crud.user.get_demo_users(db)
    assert len(users) == 2
...
async def test_user_authenticate(db: AsyncSession, cleandb) -> None:
    ...
    user = await cr_rand_user(db, email="test@test.test", password="test")
